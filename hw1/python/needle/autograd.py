...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for var in reverse_topo_order:
      var.grad = array_api.sum(node_to_output_grads_list[var])
      if var.op is not None:
        grad = var.op.gradient(var.grad, var)
      for k_ in range(len(var.inputs)):
        if var.inputs[k_] in node_to_output_grads_list:
          node_to_output_grads_list[var.inputs[k_]].append(grad[k_])
        else:
          node_to_output_grads_list[var.inputs[k_]] = [grad[k_]]
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    ### BEGIN YOUR SOLUTION
    res, visited = [], set()
    for i in range(len(node_list[0].inputs)):
      topo_sort_dfs(node_list[0].inputs[i], visited, res)
